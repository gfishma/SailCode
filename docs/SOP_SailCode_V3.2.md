...
**LP 通路**：DAC → Output Side 开关矩阵 → T16 → 通过 `switch set(X, Y2, T16, ON)` 把电压路由到任意 X 通道
...
```
# LP: 输出 2.5V，通过矩阵 X1→Y2→T16 接出，再 em_dmm(X1) 测量
>em_cvs set(LP, 2.5V)
...
# HP: 输出 15V，从 HP 专用输出端测量 (需配置 IO7=1, IO34=1, IO6=1, IO20=1, IO87=1)
>em_cvs set(HP, 15V)
...
# NP: 输出 -3V，从 NP 专用输出端测量 (需配置 IO33=1)
>em_cvs set(NP, 3V)
>em_io set([33,1])
...
| 量程 | 取样电阻 | IO1/IO2 | 电流范围 |
|------|---------|---------|---------|
| 0 | 100Ω | 00 | ~5μA ~ 10mA |
| 1 | 499Ω | 01 | ~1μA ~ 10mA |
| 2 | 10KΩ | 10 | ~0.05μA ~ 500μA |
| 3 | 1MΩ | 11 | ~0.5nA ~ 5μA |
...
- Y7/Y8 用于 HIGH_VOLT/HIGH_CURR 的 LV 测量路径
...
# 仅连接 Y8→T13 (用于 HIGH_CURR_CH LV 测量)
>switch yf set(Y8, T13, ON)
```
