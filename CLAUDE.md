...
- **RAM state**: ~44% used (~74KB free). FreeRTOS heap (64KB) moved to CCMRAM via `configAPPLICATION_ALLOCATED_HEAP=1` + custom `ucHeap` in `os.c` with `__attribute__((section(".ccmram")))`. CCMRAM is 64KB at 0x10000000 — CPU-only, zero-wait, no DMA access. Main SRAM (128KB) now has plenty of headroom. Disabled ADC/DAC (SCMD + StdPort) from build. PWM/Capture are ACTIVE.
...
- **DVM CH1** (high voltage): 0-25V range with 1/6 divider → actual = raw × 6. CVS HP measurement via IO6/7/20/34/87=1. PD_CAL measurement via IO22/87=1. Both use `>dvm get(dvm_0, 1)`.
